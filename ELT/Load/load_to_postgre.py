...
def load_data_company_to_postgres():
    ...
    if conn is not None:
        try:
            ...
            sqlStr = "insert into company (symbol, exchange , type, organ_short_name ,organ_name) values %s"
...
def load_matching_data_to_postgres(**context):
    try:
        t =datetime.now().time()
    ...
    if conn is not None:
        ...
        sqlStr = "insert into matching_trans (time , price , volume, match_type, id , symbol) values %s"


        for company in companies:

            try:
                df = extract_matching_data(symbol=company[0])
